...
- 鼠标主动点击建议项时，popup 先把 selection 送回 Widget；main process 不再抢先关闭 popup，而是由 Widget 在收到并处理 selection 后关闭，避免关闭窗口、输入框 blur 与 selection 回传互相竞态；
- Widget 收到历史 selection 时，只要当前 work entry 仍可编辑，就把该 selection 视为权威选择；即使输入框的 blur-save 已经先结束 edit mode，也不能因此丢弃鼠标选择；现有约 120 ms blur 延时仍保留用于普通 blur，但不再承担保证 History selection 正确性的职责；
...
6. Description History popup 出现时不要主动抢 Description 输入焦点，但窗口本身必须保持可交互；不要再用 `focusable: false` 破坏 Windows 鼠标选择。鼠标 selection 必须由 Widget 确认处理后再关闭 popup，且不能因为 input blur 已结束 edit mode 就丢弃已选中的历史项。
...
- Description History 是否仍可用鼠标左键和键盘选择，且 popup 出现时不会主动抢焦点；鼠标主动点击建议时应先成功应用 `Description + Project + Task`，再由 Widget 关闭 popup，不能出现“popup 消失但历史项未应用”；
